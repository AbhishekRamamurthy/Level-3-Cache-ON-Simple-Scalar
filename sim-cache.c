...
sim_reg_options(struct opt_odb_t *odb)	/* options database */
{
	...
	opt_reg_note(odb,
			...
			"      -cache:il1 dl1\n"
			"      -cache:dl1 ul1:256:32:1:l -cache:dl2 ul2:1024:64:2:l -cache:dl3 ul3:1024:64:4:l\n");

	opt_reg_string(odb, "-cache:il2",
			"l2 instruction cache config, i.e., {<config>|dl2|none}",
			&cache_il2_opt, "dl2", /* print */TRUE, NULL);
	opt_reg_string(odb, "-cache:il3",
			"l2 instruction cache config, i.e., {<config>|dl3|none}",
		...
		int argc, char **argv)	/* command line arguments */
{
	...
	else /* il1 is defined */
	{
		...
		else if (!mystricmp(cache_il2_op,"dl3")) 
		{
			if(!cache_dl3) 
				fatal("I-cache l2 cannot access D-cache l2 as it's undefined");
			cache_il2 = cache_dl3;
...
sim_reg_stats(struct stat_sdb_t *sdb)	/* stats database */
{
	...
	if (cache_il3
			&& (cache_il3 != cache_dl3 && cache_il3 != cache_dl2 && cache_il3 != cache_dl3))
